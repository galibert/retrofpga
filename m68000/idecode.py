    ...
    def elaborate(self, platform):
        ...
        with m.Switch(self.i_ird):
            ...
            with m.Case('0000---1--------'):
                m.d.comb += self.bw.eq(1)
            with m.Default():
                m.d.comb += self.bw.eq(0)

        with m.Switch(self.i_ird):
            with m.Case('1--1---11-------'):
                m.d.comb += self.l.eq(1)
            with m.Case('1--1----10------'):
                m.d.comb += self.l.eq(1)
            with m.Case('1-0-----10------'):
                m.d.comb += self.l.eq(1)
            ...
            with m.Case('0010------------'):
                m.d.comb += self.l.eq(1)
            with m.Default():
                m.d.comb += self.l.eq(0)

        m.d.comb += self.o_linea.eq(self.i_ird[12:16] == 0xa)
        m.d.comb += self.o_linef.eq(self.i_ird[12:16] == 0xf)

        m.d.comb += self.o_ma1.eq(0x000)
        with m.Switch(Cat(self.l, self.bw, self.i_ird)):
